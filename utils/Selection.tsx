...
import { Questionaire, Key } from '../pages/zkouseni'
...
export default function Selection(props: Props) {
    ...
    const moveToNextQuestion = (predefined?: string) => {
        ...
        //correct
        if (removeDiacritics(inputString.toLocaleLowerCase()).trim() === removeDiacritics(answer.toLocaleLowerCase()).trim() || inputString === "") {
            setIsCorrect(true)
            setFeedbackString(`${Lang(lang, ["Correct", "Dobře"])} ${!isSwitchable ? `${question} (${questionIndex + 1}) = ` : ""}${answer}`)
        ...
        //wrong
            ...
            setIsCorrect(false)
            setFeedbackString(`${Lang(lang, ["Wrong", "Špatně"])} ${!isSwitchable ? `${question} (${questionIndex + 1}) = ` : ""}${answer}`)
            var newResult = questionCorrectnessArray
            newResult[questionIndex] -= 1
            setQuestionCorrectnessArray(newResult)
    ...
    var isSwitchable = typeof questionaire.config.allowCustomAnswers === "undefined" ? true : questionaire.config.allowCustomAnswers
    ...
    return (
        <div className={styles.App}>
            <div className={styles.container}>
                <div className={`${styles.selectionRow} ${styles.firstSelectionRow}`}>
                    {questionaire.config.useImages ? <img className={styles.questionImage} src={question} alt="Question" /> : <div className={styles.zadani}>{`${capitalizeFirst(question)} ${questionCorrectnessArray[questionIndex]}`}</div>}
                    <button className={`button button-clear`} onClick={onOpenerClick}>{`⚙️`}</button>
                </div>
                <div className={`${styles.selectionRow} space-between`} style={{ marginTop: ".5em" }}>
                ...
                <div className={`${styles.selectionRow} ${styles.lastSelectionRow}`}>
                    {isSwitchable && <button className="button" onClick={moveToNextQuestionVoid}>{Lang(lang, ["Confirm", "Potvrdit"])}</button>}
                    <div className={`${styles.resultText} ${styles[`correctness-${isCorrect}`]}`} >{feedbackString}</div>
                </div>
                <Settings
...
function removeDiacritics(str: string) {
    ...
    return str.replace(/[^\u0000-\u007E]/g, function (a: any) {
        //@ts-expect-error
